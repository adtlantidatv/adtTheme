...
function serializeParams(e){for(var n in e){var r=e[n];r instanceof MediaObject&&(e[n]=r.id)}return e}function getConstructor(e){return core[e]||endpoints[e]||filters[e]||hubs[e]||abstracts[e]||MediaObject}function createConstructor(e){var n=getConstructor(e.type);if(n.create){e=n.create(e.params);var r=n.prototype;inherits(n,getConstructor(e.type)),extend(n.prototype,r)}return n.item=e,n}function createPromise(e,n){return new Promise(function(r,t){function i(e,n){return e?t(e):void r(n)}e instanceof Array?async.map(e,n,i):n(e,i)})}function KwsMedia(e,n,r){function t(e){var n=e.method,r=e.params.value,t=r.object,i=s[t];if(!i)return console.warn("Unknown object id '"+t+"'",e);switch(n){case"onEvent":i.emit(r.type,r.data);break;default:console.warn("Unknown message type '"+n+"'")}}function i(e,n,r){var t=new e(n,r);return t.on("release",function(){delete s[n]}),t.on("_rpc",function(e,r,t){r.object=n,r.operationParams=serializeParams(r.operationParams),d.encode(e,r,function(e,n){if(e)return t(e);var i=r.operation;if("getConnectedSinks"==i||"getMediaSinks"==i||"getMediaSrcs"==i){var o=n.sessionId;return a.getMediaobjectById(n.value,function(e,n){var n={sessionId:o,value:n};t(e,n)})}t(null,n)})}),(t instanceof Hub||t instanceof MediaPipeline)&&t.on("_create",a.create.bind(a)),s[n]=t,t}function o(e,n){var r=createConstructor(e);e=r.item,delete r.item,e.constructorParams=checkParams(e.params,r.constructorParams,e.type),delete e.params,e.constructorParams=serializeParams(e.constructorParams),d.encode("create",e,function(e,t){if(e)return n(e);var o=t.value;n(null,s[o]||i(r,o))})}function c(e,n){var r=s[e];return r?n(null,r):void d.encode("describe",{object:e},function(r,t){if(r)return n(r);var o=createConstructor(t);return delete o.item,n(null,i(o,e))})}if(!(this instanceof KwsMedia))return new KwsMedia(e,n,r);var a=this;EventEmitter.call(this),n instanceof Function&&(r=n,n=void 0),n=n||{};var s={};if("string"==typeof e){var u=n.access_token;void 0!=u&&(e=url.parse(e,!0),e.query.access_token=u,e=url.format(e),delete n.access_token)}var d=new RpcBuilder(JsonRPC,function(e){return e instanceof RpcBuilder.RpcNotification?void 0!=e.duplicated?console.warning("Unexpected request:",e):t(e):void console.error("Invalid request instance",e)}),l=reconnect(function(e){d.transport=e}).connect(e);this.close=l.disconnect.bind(l),l.on("fail",this.emit.bind(this,"disconnect")),this.then=function(e,n){return new Promise(function(r,t){function i(){var n;if(e)try{n=e(a)}catch(i){return t(i)}r(n)}function o(){var e;if(n)try{e=n(new Error("Connection error"))}catch(r){return t(r)}t(e)}l.connected?i():l.reconnect?(l.on("connection",i),l.on("fail",o)):o()})},this.catch=this.then.bind(this,null),this.then(function(e){r(null,e)},r),this.getMediaobjectById=function(e,n){var r=createPromise(e,c,n);return promiseCallback(r,n),r},this.create=function(e,n,r){if(n instanceof Function){if(r)throw new SyntaxError("Nothing can be defined after the callback");r=n,n=void 0}e instanceof Array||(e={params:n||{},type:e});var t=createPromise(e,o,r);return promiseCallback(t,r),t}}var EventEmitter=require("events").EventEmitter,extend=require("extend"),inherits=require("inherits"),url=require("url"),Promise=require("es6-promise").Promise,async=require("async"),reconnect=require("reconnect-ws"),RpcBuilder=require("kws-rpc-builder"),JsonRPC=RpcBuilder.packers.JsonRPC,checkType=require("./checkType"),checkParams=checkType.checkParams,utils=require("./utils"),noop=utils.noop,promiseCallback=utils.promiseCallback,core=require("./core"),endpoints=require("./endpoints"),filters=require("./filters"),hubs=require("./hubs"),Hub=require("./core/Hub"),MediaObject=require("./core/MediaObject"),HubPort=core.HubPort,MediaPipeline=core.MediaPipeline,abstracts={Filter:require("./core/Filter"),Hub:Hub,MediaElement:require("./core/MediaElement"),MediaPad:require("./core/MediaPad"),MediaSink:require("./core/MediaSink"),MediaSource:require("./core/MediaSource"),HttpEndpoint:require("./endpoints/HttpEndpoint"),SdpEndpoint:require("./endpoints/SdpEndpoint"),SessionEndpoint:require("./endpoints/SessionEndpoint"),UriEndpoint:require("./endpoints/UriEndpoint")};inherits(KwsMedia,EventEmitter);var checkMediaElement=checkType.bind(null,"MediaElement","media");KwsMedia.prototype.connect=function(e,n){if(e=Array.prototype.slice.call(arguments,0),n="function"==typeof e[e.length-1]?e.pop():noop,e.length<2)throw new SyntaxError("Need at least two media elements to connect");e.forEach(checkMediaElement);var r=new Promise(function(n,r){function t(e,t){return e?r(e):void n(t)}var i=e[0];async.each(e.slice(1),function(e,n){i.connect(e,n),i=e},t)});return promiseCallback(r,n),r},module.exports=KwsMedia;
...
var HttpGetEndpoint=require("./HttpGetEndpoint"),HttpPostEndpoint=require("./HttpPostEndpoint"),PlayerEndpoint=require("./PlayerEndpoint"),RecorderEndpoint=require("./RecorderEndpoint"),RtpEndpoint=require("./RtpEndpoint"),WebRtcEndpoint=require("./WebRtcEndpoint");exports.HttpGetEndpoint=HttpGetEndpoint,exports.HttpPostEndpoint=HttpPostEndpoint,exports.PlayerEndpoint=PlayerEndpoint,exports.RecorderEndpoint=RecorderEndpoint,exports.RtpEndpoint=RtpEndpoint,exports.WebRtcEndpoint=WebRtcEndpoint;
//# sourceMappingURL=kws-media-api.map
...
var EventEmitter=require("events").EventEmitter,backoff=require("backoff");module.exports=function(e){return function(n,t){function c(c,f){function a(e){if(o.connected=!1,u.removeListener("error",a),u.removeListener("close",a),u.removeListener("end",a),"Request"==u.constructor.name&&u.on("error",function(){}),o.emit("disconnect",e),o.reconnect)try{r.backoff()}catch(n){}}if(!o.connected&&o.reconnect){o.emit("reconnect",c,f);var u=e.apply(o,i);o._connection=u,u.on("error",a).on("close",a).on("end",a),n.immediate||"Request"==u.constructor.name?(o.connected=!0,o.emit("connect",u),o.emit("connection",u),u.once("data",function(){r.reset()})):u.on("connect",function(){r.reset(),o.connected=!0,t&&u.removeListener("connect",t),o.emit("connect",u),o.emit("connection",u)})}}t="function"==typeof n?n:t,n="object"==typeof n?n:{initialDelay:1e3,maxDelay:3e4},t||(t=n.onConnect);var o=new EventEmitter;o.connected=!1,o.reconnect=!0,t&&o.on("connection",t);var r=(backoff[n.type]||backoff.fibonacci)(n);r.on("backoff",function(e,n){o.emit("backoff",e,n)});var i;return o.connect=o.listen=function(){return this.reconnect=!0,o.connected?void 0:(r.reset(),r.on("ready",c),i=i||[].slice.call(arguments),c(0,0),o)},o.disconnect=function(){return this.reconnect=!1,o._connection&&o._connection.end(),o},o}};
//# sourceMappingURL=kws-media-api.map
